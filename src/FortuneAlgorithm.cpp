...
void FortuneAlgorithm::handleCircleEvent(Event* event)
{
    ...
    Arc* arc = event->arc;
    // 1. Add vertex
    Vertex* vertex = mDiagram.createVertex(point);
...
void FortuneAlgorithm::removeArc(Arc* arc, Vertex* vertex)
{
    // End edges
...
void FortuneAlgorithm::setOrigin(Arc* left, Arc* right, Vertex* vertex)
{
    left->rightHalfEdge->destination = vertex;
    right->leftHalfEdge->origin = vertex;
}

void FortuneAlgorithm::setDestination(Arc* left, Arc* right, Vertex* vertex)
{
    left->rightHalfEdge->origin = vertex;
    right->leftHalfEdge->destination = vertex;
}

void FortuneAlgorithm::setPrevHalfEdge(HalfEdge* prev, HalfEdge* next)
{
    prev->next = next;
    next->prev = prev;
}
...
bool FortuneAlgorithm::bound(Box box)
{
    ...
    if (!mBeachline.isEmpty())
    {
        ...
        while (!mBeachline.isNil(rightArc))
        {
            ...
            Vertex* vertex = mDiagram.createVertex(intersection.point);
    ...
    for (auto& kv : vertices)
    {
        ...
        for (std::size_t i = 0; i < 5; ++i)
        {
            ...
            if (cellVertices[2 * side] == nullptr && cellVertices[2 * side + 1] != nullptr)
            {
                ...
                Vertex* corner = mDiagram.createCorner(box, static_cast<Box::Side>(side));
            ...
            {
                Vertex* corner = mDiagram.createCorner(box, static_cast<Box::Side>(nextSide));
                linkedVertices.emplace_back(LinkedVertex{nullptr, corner, nullptr});
    ...
    for (auto& kv : vertices)
    {
        ...
        for (std::size_t side = 0; side < 4; ++side)
        {
            ...
            {
                // Link vertices 
                HalfEdge* halfEdge = mDiagram.createHalfEdge(mDiagram.getFace(i));
